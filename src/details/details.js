...
class Details extends Component {
    ...
    componentWillMount() {
        fetch(`http://localhost:8000/${this.convertSpace(this.props.match.params.coin)}`)
    ...
    render(){
        return (
            <div>
                {this.state.config ? <ReactHighstock config = {this.state.config}/> : null}
                
                <img alt={this.props.match.params.coin} src={`https://s2.coinmarketcap.com/static/img/coins/64x64/${this.props.match.params.id}.png`} />
            </div>
        )
    }
} 
